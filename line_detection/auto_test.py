        ...
        if event == cv2.EVENT_FLAG_LBUTTON:
            ...
            if count < 2:
                ...
                # 수정 - 직선 그리기
                    ...
                    cv2.line(now_frame, (x1, y1), (x2, y2), (0, 255, 255), 1)

                    pre_point.append((ix, iy))
                    pre_point.append((ix2, iy2))
                    now_point.append((ix, iy))
                    now_point.append((ix2, iy2))

                    # 수정
                    if len(now_point)>2:

                        # 직선이 교차할 경우 각 직선의 좌표 1개씩 추출해서 서로 바꿈
                        # 함수에 넣어야 될 것 같은데 함수 건드리면 안될거 같아서 일단 마우스 콜백에 넣어둠
                        if is_cross_pt(now_point[0][0],now_point[0][1],now_point[1][0],now_point[1][1],
                            ...
                            now_point[0] = list(now_point[0])
                            now_point[1] = list(now_point[1])
                            now_point[2] = list(now_point[2])
                            now_point[3] = list(now_point[3])
                            tx1, ty1, tx2, ty2 = find_point((now_point[0][0], now_point[0][1]),
                                                        (now_point[1][0], now_point[1][1]))
                            tx3, ty3, tx4, ty4 = find_point((now_point[2][0], now_point[2][1]),
                                                        (now_point[3][0], now_point[3][1]))

                            now_point[0][0], now_point[0][1] = tx3, ty3
                            now_point[2][0], now_point[2][1] = tx1, ty1
                            now_point[0] = tuple(now_point[0])
                            now_point[1] = tuple(now_point[1])
                            now_point[2] = tuple(now_point[2])
                            now_point[3] = tuple(now_point[3])
...
while True:
    ...
    if pause == 0:
        ...
        # 프레임 고정
            ...
            # 2 . 엣지 찾기
            canny = cv2.Canny(gray, 350, 350, apertureSize=3)
            # 3. 찾은 엣지로 직선 검출
            lines = cv2.HoughLines(canny, 1, np.pi / 180, 150)
            k=0

            # 4. 찾은 직선 좌표로 실제 프레임에 그리기
            for i in lines:
                ...
                cv2.line(frame, (x1_points[k], y1_points[k]), (x2_points[k], y2_points[k]), (0, 0, 255),1)
                k += 1
